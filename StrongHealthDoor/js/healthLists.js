var dat = [];
/*点击切换疾病和生活*/
$(".leftLabel ul li").click(function() {
	$(this).addClass('cur').siblings("li").removeClass('cur');
	var index1 = $(this).index();
	$(".flashRecom h2").removeClass("cur");
	$(".contentWrap .main-box-nick").eq(index1).show().siblings(".main-box-nick").hide();
...
$(document).ready(function() {
	...
	switch(cur) {
		case '0':
			break;
		case '1':
			$(".leftLabel ul li").eq(1).addClass("cur").siblings().removeClass('cur');
			$(".contentWrap .main-box-nick").eq(1).show().siblings(".main-box-nick").hide();
			getData(cur);
			break;
		case '2':
			$(".leftLabel ul li").eq(2).addClass("cur").siblings().removeClass('cur');
			$(".contentWrap .main-box-nick").eq(2).show().siblings(".main-box-nick").hide();
			getData(cur);
			break;
		case '3':
			$(".leftLabel ul li").removeClass('cur');
			$(".flashRecom h2").addClass("cur");
		default:
			break;
	}
});
...
$(document).ready(function() {
	$.ajax({
		...
		success: function(res) {
			...
			var con = '';
			$.each(resrev, function(i, v) {
				con += '<a href="healthDetails.html?Nid=' + v.id + '" target="_blank">' +
					'<li class="blue">' +
					'<p>' + v.title + '</p>' +
					'</li>' +
					'</a>';
			});
			$('.scrollbar').append(con);
		},
		error: function(res) {
			console.log("error");
		}
...
function paginationNick(opt) {
	...
	function goPage(btn) {
		...
		arr.forEach(function(v) {
			str += '<div class="thinkTankTag clearfix"><a target="_blank" href="healthDetails.html?nid=' + v.id + '">' +
	...
	function createOtherBtn() {
		$('.' + pager.paginationBox).html('<div class="' + pager.btnBox + '"><button data-page="first" class="first-btn">首页</button><button data-page="prev" class="prev-btn"><<</button><span class="' + pager.numBtnBox + '"></span><button data-page="next" class="next-btn">>></button><button class="twig">共' + dat.length + '条</button><input type="text" placeholder="请输入页码" class="' + pager.ipt + '"><button class="' + pager.goBtn + '">跳转</button><button data-page="last" class="last-btn">尾页</button></div><div class="' + pager.mainBox + '"></div>');
	...
	function createNumBtn(page) {
		...
		//按钮禁用
		$('.' + pager.btnBox + ' button').not('.' + pager.currentBtn).attr('disabled', false).removeClass("btnStyle");
		if(!page) { //首页时
			$('.' + pager.btnBox + ' .first-btn').attr('disabled', true).addClass("btnStyle");
			$('.' + pager.btnBox + ' .prev-btn').attr('disabled', true).addClass("btnStyle");
		}
		if(page == pager.maxCount - 1) { //尾页时
			$('.' + pager.btnBox + ' .last-btn').attr('disabled', true).addClass("btnStyle");
			$('.' + pager.btnBox + ' .next-btn').attr('disabled', true).addClass("btnStyle");
		}
	}
...
function paginationNick1(opt) {
	...
	function goPage(btn) {
		...
		arr = pager.data.slice(pager.pageCount * pager.currentPage,
			...
			pager.pageCount * (pager.currentPage + 1) : pager.data.length);
		arr.forEach(function(v) {
			str += '<div class="thinkTankTag clearfix"><a target="_blank" href="healthDetails.html?did=' + v.id + '">' +
				'<div class="img pull-left">' +
				'<img src="' + v.img + '">' +
				'</div>' +
				'<div class="text pull-right">' +
...
function paginationNick2(opt) {
	...
	function goPage(btn) {
		...
		arr = pager.data.slice(pager.pageCount * pager.currentPage,
			...
			pager.pageCount * (pager.currentPage + 1) : pager.data.length);
		arr.forEach(function(v) {
			str += '<div class="thinkTankTag clearfix"><a target="_blank" href="healthDetails.html?lid=' + v.id + '">' +
				'<div class="img pull-left">' +
				'<img src="' + v.img + '">' +
				'</div>' +
				'<div class="text pull-right">' +
